...
public class Piece {
    ...
    public void respondToKey(int key) {
        switch (key) {
            case KeyEvent.VK_W, KeyEvent.VK_UP -> this.rotate();
            case KeyEvent.VK_A, KeyEvent.VK_LEFT -> velocityX = -1;
            case KeyEvent.VK_D, KeyEvent.VK_RIGHT -> velocityX = 1;
            case KeyEvent.VK_S, KeyEvent.VK_DOWN -> velocityY = 1;
        }
    }
    ...
    private void checkLines() {
        ...
        for (int r = ROWS - 1; r > 0; r--) seeIfLineIsFullAndThenRemove(r);
    }
    ...
    public boolean canMove(int destX, int destY) {
        for (var block : blocks) {
            ...
            if (x < 0 || x >= COLUMNS || y < 0 || y >= ROWS || tetrisPanel.get(x, y) != null) return false;
        }
        return true;
    }
    ...
    private void fillGrid() {
        ...
        if (!canMove(0, velocityY)) {
            ...
            for (var block : blocks) tetrisPanel.fill(block.blockX, block.blockY, color);
    ...
    private boolean canRotate(Block centerBlock) {
        for (var block : blocks) {
            ...
            if (nextY >= ROWS || nextX < 0 || nextX >= COLUMNS || tetrisPanel.get(nextX, nextY) != null) return false;
        }
        return true;
    }
    ...
    private void seeIfLineIsFullAndThenRemove(int line) {
        ...
        for (int i = 0; i < COLUMNS; i++) tetrisPanel.fill(i, line, null);
    ...
    private void repositionBlocks(int line) {
        for (int c = 0; c < COLUMNS; c++)
            for (int r = line; r > 0; r--) tetrisPanel.fill(c, r, tetrisPanel.get(c, r - 1));
    }
